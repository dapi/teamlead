...
1. отделить определение причины отказа от форматирования сообщения;
2. отделить project snapshot lookup от repo-level issue lookup.
...
- доменный слой получает структурированное решение об отказе и собирает
  единое user-facing сообщение.
...
- форматирование сообщения использует именно этот список.
...
- источником истины о project status остается snapshot configured GitHub
  Project;
- repo-level lookup issue не заменяет `ProjectSnapshot`, а дополняет его только
...
- `ai-teamlead run <issue>` начинает печатать более подробные ошибки;
- для `IssueNotInProject` в сообщении появляется подсказка формата
  `gh project item-add ... --url <ISSUE_URL>`;
- для `StatusDenied` сообщение показывает текущее значение status field и
  разрешенные entry statuses;
- при этом фактические GitHub mutations не добавляются: команда остается
  read-only до момента реального launch path.

Отдельное ограничение:

- configured `github.project_id` не равен numeric project number для `gh project
  item-add`, поэтому formatter не должен притворяться, что знает недостающий
  номер; либо используется placeholder, либо отдельно добавляется lookup, если
  это окажется дешевым и надежным.

## Architecture Notes

- Не стоит решать задачу простым расширением текущей строки `reason`.
  Структурированный тип отказа лучше соответствует contract-first подходу и
  quality bar проекта.
...
- обновятся только тексты отказов и coverage для них;
