# Feature 0001: ai-teamlead daemon
...
Эта feature описывает первую версию `ai-teamlead` как самостоятельного
foreground daemon, который работает в контексте одного репозитория, периодически
ищет подходящие GitHub issues и запускает `issue-analysis-flow`.
...
- создан каталог feature 0001 для `ai-teamlead daemon`
