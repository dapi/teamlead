...
оборачиванию его в loop для публичной команды `loop`.
...
- оставить CLI-команду `poll` thin wrapper над этим helper, чтобы ее контракт не
  изменился
- реализовать `run_loop`, чтобы после успешного bootstrap она входила в
  бесконечный цикл:
...
- не пробрасывать ошибки одного cycle наружу из `loop`; вместо этого
  логировать их и продолжать следующую итерацию

Это сохраняет один источник бизнес-логики для `poll` и `loop` и не создает
второй расходящийся execution path.

## Затронутые области

- `src/app.rs`
  `loop` entrypoint, выделение общего polling cycle, обработка loop и
  диагностики
...
CLI shape (`poll`, `run`, `loop`) фиксируется отдельным ADR и остается
стабильной после этого изменения.
...
лог для `poll` и `loop` и позволит тестировать loop без парсинга случайных
строк.
...
- `loop` должен продолжать работать, пока процесс не остановлен внешним
...
- не дублировать polling logic между `poll` и `loop`
- четко разделить bootstrap-ошибки и cycle-ошибки
- сделать диагностику loop явной, а не выводить только стартовые параметры

## Risks

- если sleep будет пропускаться после ошибки, `loop` превратится в busy loop
- если `loop` получит отдельный polling path, `poll` и `loop` начнут
...
- stdout/stderr как операторская диагностика foreground `loop`

Изменение касается только частоты и устойчивости их вызова во времени.

## ADR Impact

Issue реализует и доводит до кода решения, зафиксированные как минимум в:
...
### Оставить отдельную реализацию `poll` и добавить новый loop-only cycle

Отклонено, потому что это создаст два почти одинаковых path с высоким риском
дрейфа контрактов и тестов.

### Завершать `loop` при любой ошибке цикла
...
`loop` с собственным polling loop.
