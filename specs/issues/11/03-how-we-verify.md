...
1. Если пользователь запускает `run` для несуществующей issue текущего
   репозитория, сообщение явно говорит, что issue не найдена.
2. Если issue существует, но закрыта, сообщение показывает `CLOSED`-состояние и
   объясняет, что `run` работает только для открытых issue.
3. Если issue открыта, но не находится в configured GitHub Project, сообщение
   содержит номер issue, `github.project_id`, URL issue и подсказку, как
   добавить issue в проект; если formatter использует команду `gh project
   item-add`, она не должна подставлять вымышленный project number.
4. Если issue находится в проекте, но ее текущий project status не допускает
   `run`, сообщение показывает фактический статус и полный список допустимых
   entry status для stage-aware dispatcher.
...
6. Для нового formatter-а и отказных веток есть unit-тесты.
7. Для CLI `run` есть integration coverage хотя бы на один case `not in
   project` или `status denied`, чтобы проверить реальный операторский вывод.
...
- integration coverage проверяет наблюдаемое CLI-сообщение, а не только факт
  ошибки.
...
- отсутствие issue в `ProjectSnapshot` само по себе не означает ни
  `not found`, ни `closed`;
- formatter ошибок использует структурированные данные, а не ветвится по
  произвольным ad-hoc строкам;
...
- formatter для `IssueNotInProject` включает `project_id` и `issue_url`;
- formatter для `StatusDenied` включает `current_status` и полный список
  `allowed_statuses`;
...
- `run` на issue вне project snapshot, но существующей в репозитории, выдает
  понятную подсказку про `github.project_id` и `gh project item-add`;
- `run` на issue с недопустимым статусом выводит current status и допустимые
  entry statuses;
...
- убедиться, что сообщение читается без обращения к исходникам и содержит
  явный следующий шаг для оператора.
...
- отказ `run` должен явно различать тип ветки хотя бы в тексте финальной
  ошибки;
- сообщения должны включать диагностические факты, которые оператор реально
  может использовать: `project_id`, current status, issue state, issue URL;
